...
try:
    all_symptoms = sorted(get_symptoms(animal))
...
# Filter out symptoms, but exclude the ones already selected
current_selection = st.session_state.get("symptoms", [])
if search:
    filtered = [s for s in all_symptoms if search.lower() in s.lower() and s not in current_selection]
else:
    filtered = [s for s in all_symptoms if s not in current_selection]
